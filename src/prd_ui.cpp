...
const bool COMMON_FIELD_IS_INTEGER[] = {
    false, false, true, true, false, false, false,
    false, false, false, false, false, true, true
};

constexpr int COMMON_FIELD_COUNT = sizeof(COMMON_FIELD_NAMES) / sizeof(COMMON_FIELD_NAMES[0]);
...
inline bool isObjReady(lv_obj_t *obj) {
...
void setNotice(lv_obj_t *label, const char *text, lv_color_t color = lv_color_hex(0xffd6d6d6)) {
    if (!label) {
        return;
    }
...
lv_obj_t *createButton(lv_obj_t *parent, const char *text, lv_coord_t x, lv_coord_t y, lv_coord_t w, lv_coord_t h, lv_event_cb_t cb, void *userData = nullptr) {
    lv_obj_t *button = lv_button_create(parent);
    lv_obj_set_pos(button, x, y);
    lv_obj_set_size(button, w, h);
    lv_obj_set_style_radius(button, 8, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(button, lv_color_hex(0x1f5ea8), LV_PART_MAIN | LV_STATE_DEFAULT);
...
lv_obj_t *createRightPanel(lv_obj_t *screen) {
    ...
    lv_obj_set_style_bg_color(panel, lv_color_hex(0x11151a), LV_PART_MAIN | LV_STATE_DEFAULT);
...
void updateErrorFrames(const DisplayComms::Status &status) {
    ...
    lv_obj_t *panels[] = {ui.rightPanelMain, ui.rightPanelMould, ui.rightPanelCommon};

    for (lv_obj_t *panel : panels) {
        if (!panel) {
            continue;
        }
        lv_obj_set_style_border_width(panel, hasError ? 4 : 0, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_border_color(panel, lv_color_hex(0xffc62828), LV_PART_MAIN | LV_STATE_DEFAULT);
    }
    ...
    if (ui.mainErrorLabel) {
        if (hasError) {
            ...
            if (status.errorMsg[0] != '\0') {
                snprintf(text, sizeof(text), "ERROR 0x%X: %s", status.errorCode, status.errorMsg);
            } else {
...
void updateLeftReadouts(const DisplayComms::Status &status) {
    ...
    snprintf(posText, sizeof(posText), "%.2f cm3", turnsToCm3(status.encoderTurns));
...
void onNavigate(lv_event_t *event) {
    ...
    eez_flow_set_screen(static_cast<int16_t>(target), LV_SCR_LOAD_ANIM_NONE, 0, 0);
}

void onStateActionQueryState(lv_event_t *) {
    DisplayComms::sendQueryState();
}

void onStateActionQueryError(lv_event_t *) {
    DisplayComms::sendQueryError();
}

void onMouldProfileSelect(lv_event_t *event) {
    int index = static_cast<int>(reinterpret_cast<intptr_t>(lv_event_get_user_data(event)));
    if (index < 0 || index >= ui.mouldProfileCount) {
        return;
    }

    uint32_t now = millis();
    bool isDoubleTap = (ui.lastTappedMould == index) && ((now - ui.lastTapMs) <= DOUBLE_TAP_MS);
    ...
    for (int i = 0; i < ui.mouldProfileCount; i++) {
        ...
        if (i == ui.selectedMould) {
            lv_obj_set_style_bg_color(ui.mouldProfileButtons[i], lv_color_hex(0x2d7dd2), LV_PART_MAIN | LV_STATE_DEFAULT);
        } else {
            lv_obj_set_style_bg_color(ui.mouldProfileButtons[i], lv_color_hex(0x26303a), LV_PART_MAIN | LV_STATE_DEFAULT);
        }
    }
...
void syncMouldSendEditEnablement() {
    bool hasSelection = ui.selectedMould >= 0 && ui.selectedMould < ui.mouldProfileCount;
    setButtonEnabled(ui.mouldButtonEdit, hasSelection);
    setButtonEnabled(ui.mouldButtonSend, hasSelection && DisplayComms::isSafeForUpdate());
...
void rebuildMouldList() {
    ...
    int y = 8;
    for (int i = 0; i < ui.mouldProfileCount; i++) {
        const char *name = ui.mouldProfiles[i].name[0] != '\0' ? ui.mouldProfiles[i].name : "Unnamed Mould";
        lv_obj_t *button = createButton(ui.mouldList, name, 8, y, 286, 46, onMouldProfileSelect, reinterpret_cast<void *>(static_cast<intptr_t>(i)));
        lv_obj_set_style_bg_color(button, lv_color_hex(0x26303a), LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_border_width(button, 1, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_border_color(button, lv_color_hex(0x41505f), LV_PART_MAIN | LV_STATE_DEFAULT);
...
void onMouldSend(lv_event_t *) {
    ...
    if (!DisplayComms::isSafeForUpdate()) {
        setNotice(ui.mouldNotice, "Unsafe machine state for MOULD send.", lv_color_hex(0xffff7a));
        return;
    }
    ...
    if (DisplayComms::sendMould(ui.mouldProfiles[ui.selectedMould])) {
        ...
        setNotice(ui.mouldNotice, "Failed to send MOULD command.", lv_color_hex(0xffff7a));
    }
}
...
void onMouldEdit(lv_event_t *) {
    ...
    setNotice(ui.mouldNotice, "Edit flow pending; PRD edit screen still to be implemented.", lv_color_hex(0xffff7a));
}
...
void onMouldNew(lv_event_t *) {
    ...
    snprintf(newProfile.name, sizeof(newProfile.name), "Local %d", ui.mouldProfileCount + 1);
    newProfile.mode[0] = '2';
    newProfile.mode[1] = 'D';
    newProfile.mode[2] = '\0';
    ...
    rebuildMouldList();
    setNotice(ui.mouldNotice, "Created local mould profile.", lv_color_hex(0xff9be7a5));
}
...
double commonFieldValue(const DisplayComms::CommonParams &common, int fieldIndex) {
    switch (fieldIndex) {
        case 0: return common.trapAccel;
        case 1: return common.compressTorque;
        case 2: return common.microIntervalMs;
        case 3: return common.microDurationMs;
        case 4: return common.purgeUp;
        case 5: return common.purgeDown;
        case 6: return common.purgeCurrent;
        case 7: return common.antidripVel;
        case 8: return common.antidripCurrent;
        case 9: return common.releaseDist;
        case 10: return common.releaseTrapVel;
        case 11: return common.releaseCurrent;
        case 12: return common.contactorCycles;
        case 13: return common.contactorLimit;
        default: return 0.0;
    }
}

void assignCommonField(DisplayComms::CommonParams &common, int fieldIndex, const char *text) {
    if (!text) {
        return;
    }
    if (COMMON_FIELD_IS_INTEGER[fieldIndex]) {
        ...
        switch (fieldIndex) {
            case 2: common.microIntervalMs = value; break;
            case 3: common.microDurationMs = value; break;
            case 12: common.contactorCycles = value; break;
            case 13: common.contactorLimit = value; break;
            default: break;
        }
        return;
    }
    ...
    switch (fieldIndex) {
        case 0: common.trapAccel = value; break;
        case 1: common.compressTorque = value; break;
        case 4: common.purgeUp = value; break;
        case 5: common.purgeDown = value; break;
        case 6: common.purgeCurrent = value; break;
        case 7: common.antidripVel = value; break;
        case 8: common.antidripCurrent = value; break;
        case 9: common.releaseDist = value; break;
        case 10: common.releaseTrapVel = value; break;
        case 11: common.releaseCurrent = value; break;
        default: break;
    }
}
...
void syncCommonInputsFromModel(const DisplayComms::CommonParams &common) {
    ...
    for (int i = 0; i < COMMON_FIELD_COUNT; i++) {
        ...
        if (COMMON_FIELD_IS_INTEGER[i]) {
            snprintf(buffer, sizeof(buffer), "%lu", static_cast<unsigned long>(commonFieldValue(common, i)));
        } else {
...
void syncCommonSendEnablement() {
    setButtonEnabled(ui.commonButtonSend, ui.commonDirty && DisplayComms::isSafeForUpdate());
}
...
void onCommonFieldChanged(lv_event_t *event) {
    ...
    if (lv_event_get_code(event) == LV_EVENT_VALUE_CHANGED || lv_event_get_code(event) == LV_EVENT_READY) {
        ui.commonDirty = true;
...
bool sendCommonFromInputs() {
    if (!DisplayComms::isSafeForUpdate()) {
        setNotice(ui.commonNotice, "Unsafe machine state for COMMON send.", lv_color_hex(0xffff7a));
        return false;
    }
    ...
    if (DisplayComms::sendCommon(toSend)) {
        setNotice(ui.commonNotice, "COMMON command sent.", lv_color_hex(0xff9be7a5));
        ui.commonDirty = false;
        syncCommonSendEnablement();
        return true;
    }

    setNotice(ui.commonNotice, "Failed to send COMMON command.", lv_color_hex(0xffff7a));
    return false;
}

void onCommonSend(lv_event_t *) {
    sendCommonFromInputs();
}
...
void createLeftReadouts(lv_obj_t *screen, lv_obj_t **posLabel, lv_obj_t **tempLabel) {
    *posLabel = lv_label_create(screen);
    lv_obj_set_pos(*posLabel, LEFT_X, 10);
    lv_obj_set_size(*posLabel, LEFT_WIDTH, LV_SIZE_CONTENT);
    lv_obj_set_style_text_align(*posLabel, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(*posLabel, &lv_font_montserrat_16, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    *tempLabel = lv_label_create(screen);
    ...
    lv_obj_set_style_text_align(*tempLabel, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(*tempLabel, &lv_font_montserrat_16, LV_PART_MAIN | LV_STATE_DEFAULT);
...
void createMainPanel() {
    ...
    lv_obj_set_style_text_font(stateHeader, &lv_font_montserrat_16, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(stateHeader, lv_color_hex(0xff9fb2c7), LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_text_font(ui.stateValue, &lv_font_montserrat_30, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_label_set_text(ui.stateValue, "--");

    ui.stateAction1 = createButton(ui.rightPanelMain, "Refresh State", 18, 235, 150, 52, onStateActionQueryState);
    ui.stateAction2 = createButton(ui.rightPanelMain, "Refresh Error", 182, 235, 150, 52, onStateActionQueryError);

    createButton(ui.rightPanelMain, "Mould Settings", 18, 720, 150, 58, onNavigate, reinterpret_cast<void *>(static_cast<intptr_t>(SCREEN_ID_MOULD_SETTINGS)));
    createButton(ui.rightPanelMain, "Common Settings", 182, 720, 150, 58, onNavigate, reinterpret_cast<void *>(static_cast<intptr_t>(SCREEN_ID_COMMON_SETTINGS)));
    ...
    lv_obj_set_style_text_font(ui.mainErrorLabel, &lv_font_montserrat_16, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(ui.mainErrorLabel, lv_color_hex(0xffff6b6b), LV_PART_MAIN | LV_STATE_DEFAULT);
...
void createMouldPanel() {
    ...
    lv_obj_set_style_bg_color(ui.mouldList, lv_color_hex(0x1a222b), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_color(ui.mouldList, lv_color_hex(0x3a4a5a), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(ui.mouldList, 1, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_text_color(ui.mouldNotice, lv_color_hex(0xffd6d6d6), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_label_set_text(ui.mouldNotice, "Select a profile.");

    ui.mouldButtonBack = createButton(ui.rightPanelMould, "Back", 18, 648, 96, 52, onNavigate, reinterpret_cast<void *>(static_cast<intptr_t>(SCREEN_ID_MAIN)));
    ui.mouldButtonSend = createButton(ui.rightPanelMould, "Send", 127, 648, 96, 52, onMouldSend);
    ui.mouldButtonEdit = createButton(ui.rightPanelMould, "Edit", 236, 648, 96, 52, onMouldEdit);
    ui.mouldButtonNew = createButton(ui.rightPanelMould, "New", 70, 712, 96, 52, onMouldNew);
    ui.mouldButtonDelete = createButton(ui.rightPanelMould, "Delete", 184, 712, 96, 52, onMouldDelete);
}
...
void createCommonPanel() {
    ...
    lv_obj_set_style_bg_color(ui.commonScroll, lv_color_hex(0x1a222b), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_color(ui.commonScroll, lv_color_hex(0x3a4a5a), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(ui.commonScroll, 1, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    for (int i = 0; i < COMMON_FIELD_COUNT; i++) {
        ...
        lv_textarea_set_accepted_chars(input, COMMON_FIELD_IS_INTEGER[i] ? "0123456789" : "0123456789.-");
        lv_textarea_set_text(input, "0");
        lv_obj_set_style_text_align(input, LV_TEXT_ALIGN_RIGHT, LV_PART_MAIN | LV_STATE_DEFAULT);
        ...
        lv_obj_add_event_cb(input, onCommonFieldChanged, LV_EVENT_VALUE_CHANGED, reinterpret_cast<void *>(static_cast<intptr_t>(i)));
        lv_obj_add_event_cb(input, onCommonFieldChanged, LV_EVENT_READY, reinterpret_cast<void *>(static_cast<intptr_t>(i)));
    ...
    lv_obj_set_style_text_color(ui.commonNotice, lv_color_hex(0xffd6d6d6), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_label_set_text(ui.commonNotice, "Edit and press Send.");

    ui.commonButtonBack = createButton(ui.rightPanelCommon, "Back", 18, 720, 150, 58, onCommonBack);
    ui.commonButtonSend = createButton(ui.rightPanelCommon, "Send", 182, 720, 150, 58, onCommonSend);
    ...
    lv_obj_set_style_bg_color(ui.commonDiscardOverlay, lv_color_hex(0x000000), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(ui.commonDiscardOverlay, LV_OPA_60, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(ui.commonDiscardOverlay, 0, LV_PART_MAIN | LV_STATE_DEFAULT);

    lv_obj_t *dialog = lv_obj_create(ui.commonDiscardOverlay);
    lv_obj_set_size(dialog, 300, 180);
    lv_obj_center(dialog);
    lv_obj_set_style_bg_color(dialog, lv_color_hex(0x1f2630), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_color(dialog, lv_color_hex(0x4a5d71), LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_text_align(dialogText, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
...
void init() {
    ...
    if (!isObjReady(objects.main) || !isObjReady(objects.mould_settings) || !isObjReady(objects.common_settings)) {
        Serial.printf(
            "PRD_UI: screen validity m=%d ms=%d cs=%d\n",
    ...
    createLeftReadouts(objects.mould_settings, &ui.posLabelMould, &ui.tempLabelMould);
    createLeftReadouts(objects.common_settings, &ui.posLabelCommon, &ui.tempLabelCommon);
    uiYield();
    ...
    ui.mouldProfileCount = 1;
    strncpy(ui.mouldProfiles[0].name, "Awaiting QUERY_MOULD", sizeof(ui.mouldProfiles[0].name) - 1);

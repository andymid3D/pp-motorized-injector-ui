...
void tick_user_widget_slider_encoder(void *flowState, int startWidgetIndex) {
    ...
    (void)startWidgetIndex;
    {
        ...
        int32_t cur_val = lv_slider_get_min_value(((lv_obj_t **)&objects)[startWidgetIndex + 0]);
        if (new_val != cur_val) {
            tick_value_change_obj = ((lv_obj_t **)&objects)[startWidgetIndex + 0];
            int16_t min = new_val;
            int16_t max = lv_slider_get_max_value(((lv_obj_t **)&objects)[startWidgetIndex + 0]);
            if (min < max) {
                lv_slider_set_range(((lv_obj_t **)&objects)[startWidgetIndex + 0], min, max);
            }
            tick_value_change_obj = NULL;
        }
    }
